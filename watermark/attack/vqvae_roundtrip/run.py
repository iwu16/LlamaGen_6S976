"""
Attack 2: VQ-VAE decode -> re-encode round trip.
...
def run_attack(root: Path, output_dir: Path) -> None:
    ...
    raise NotImplementedError("Implement VQ-VAE decode/re-encode here.")


def main() -> None:
    parser = argparse.ArgumentParser()
    parser.add_argument("--root", type=Path, default=Path("aggregated_samples"))
    parser.add_argument("--output-dir", type=Path, default=Path("attack_outputs/vqvae_roundtrip"))
    parser.add_argument("--results", type=Path, default=Path("results/vqvae_roundtrip.json"))
    ...
    report = {
        "baseline_detection": compute_tpr_fpr(
            args.root,
            args.root,
            csv_dir=Path("results/detection_csv/vqvae_roundtrip"),
        ),
