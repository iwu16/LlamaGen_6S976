...
def run_attack(root: Path, output_dir: Path) -> None:
    ...
    raise NotImplementedError("Implement token-by-token regeneration here.")


def main() -> None:
    parser = argparse.ArgumentParser()
    parser.add_argument("--root", type=Path, default=Path("aggregated_samples"))
    parser.add_argument("--output-dir", type=Path, default=Path("attack_outputs/token_regeneration"))
...
#    run_attack(args.root, args.output_dir)
    report = {
        "baseline_detection": compute_tpr_fpr(
            args.root,
            args.root,
            csv_dir=Path("results/detection_csv/token_regeneration"),
        ),
